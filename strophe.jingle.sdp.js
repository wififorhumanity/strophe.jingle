...
SDP.prototype.toJingle = function (elem, thecreator) {
    ...
    for (i = 0; i < this.media.length; i++) {
        ...
        if (SDPUtil.find_line(this.media[i], 'a=rtpmap:').length) {
          ...
          elem.c('transport', {xmlns: 'urn:xmpp:jingle:transports:ice-udp:1'});
          for (j = 0; j < mline.fmt.length; j++) {
                sctpmap = SDPUtil.find_line(this.media[i], 'a=sctpmap:' + mline.fmt[j]);
                var attr = SDPUtil.parse_sctpmap(sctpmap);
...
SDPUtil = {
    ...
    build_sctpmap: function (el) {
        var line = 'a=sctpmap:' + $(el).attr('number') + ' ' + $(el).attr('protocol') + ' ' + $(el).attr('streams');
        return line;
    },
